...
OpencastUploader.prototype = {
  ...
  loginAndUploadFromAnchor: async function(anchors, onsuccess, onloginfailed, onserverunreachable,
    oninetorpermfailed, title, creator) {
    // relog needed?
    let isLoggedIn = true;
    try {
    ...
    this.uploadFromAnchor(anchors, onsuccess, onloginfailed, onserverunreachable,
      oninetorpermfailed, title, creator);
  },
  uploadFromAnchor: async function(anchors, onsuccess, onnotloggedin, onserverunreachable,
    oninetorpermfailed, title, creator) {
    ...
    try {
      let mp = await this.getMediaPackage()
        .then(mp => {
          ...
          return this.addDCCatalog(mp, new XMLSerializer().serializeToString(dc), "dublincore/episode");
        });
  ...
  login: function() {
    let data = "j_username=" + this.login_name +
      "&j_password=" + this.login_password +
...
function OpencastUploaderSettingsDialog() {
  ...
  this.serverURLEl = document.querySelector('#ocUploadSettings input[name=ocUploadSettingsServerURL]');
  this.workflowIdEl = document.querySelector("#ocUploadSettings input[name=ocUploadSettingsWorkflowId]");
  this.loginNameEl = document.querySelector('#ocUploadSettings input[name=ocUploadSettingsLoginName]');
  this.loginPasswordEl = document.querySelector("#ocUploadSettings input[name=ocUploadSettingsLoginPassword]");
  ...
  // bind save button
  this.saveOcUploadSettingsEl.addEventListener("click", this.saveAndCloseOcUploadSettings.bind(this), false);
}
